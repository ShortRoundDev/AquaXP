...
namespace AquaXP
{
    ...
    public:
        ...
        AQUAXP_API Texture(ID3D11Device* device, ID3D11DeviceContext* context, f32 width, f32 height, D3D11_BIND_FLAG flags = D3D11_BIND_SHADER_RESOURCE);

        AQUAXP_API ~Texture();

        AQUAXP_API void use(Graphics const* graphics, u32 slot = TEX_ALBEDO) const;

        AQUAXP_API ID3D11Resource* getResource() const;
        AQUAXP_API ID3D11Texture2D* getTexture2D() const;
        AQUAXP_API ID3D11ShaderResourceView* getShaderResourceView() const;
        AQUAXP_API ID3D11DepthStencilView* getDepthStencilView() const;
        AQUAXP_API ID3D11RenderTargetView* getRenderTargetView() const;
