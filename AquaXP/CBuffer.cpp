...
public:
    ...
    bool bind(ID3D11DeviceContext* context, UINT slot = 0, CbufferBindStage bindStage = VS | PS) const
    {
        T copy;
...
CBuffer<T>::CBuffer(
    ...
    m_pimpl(make_unique<CBuffer<T>::impl>(device, cBufferData))
{
...
bool CBuffer<T>::bind(
    ...
    UINT slot,
    CbufferBindStage bindStage
) const
{
    return m_pimpl->bind(context, slot, bindStage);
}

template<typename T>
void CBuffer<T>::setData(T const& data)
{
    m_pimpl->setData(data);
}

template<typename T>
T const& CBuffer<T>::getData() const
{
    return m_pimpl->getData();
}

template<typename T>
T& CBuffer<T>::getDataMutable()
{
    return m_pimpl->getDataMutable();
}
